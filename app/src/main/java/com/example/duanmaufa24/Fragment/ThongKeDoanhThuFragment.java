    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        btnRevenue.setOnClickListener(v -> {
            ...
            if (!TextUtils.isEmpty(startDate) && !TextUtils.isEmpty(endDate) &&
                    !startDate.equals("Chưa chọn") && !endDate.equals("Chưa chọn")) {
                try {
                    // Kiểm tra nếu ngày bắt đầu trước ngày kết thúc
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
                    if (sdf.parse(startDate).after(sdf.parse(endDate))) {
                        tvRevenue.setText("Ngày bắt đầu phải trước ngày kết thúc.");
                    } else {
                        double totalRevenue = dbHelper.getTongTienThuTrongKhoangThoiGian(startDate, endDate);
                        tvRevenue.setText("Doanh Thu: " + totalRevenue + " VNĐ");
                    }
                } catch (Exception e) {
                    tvRevenue.setText("Lỗi định dạng ngày.");
                }
            } else {
